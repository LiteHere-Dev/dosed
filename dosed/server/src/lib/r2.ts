...
export async function getObject(key: string): Promise<Response> {
  if (!client) throw new Error("R2 not configured");
  const signed = await client.sign(objectUrl(key), { method: "GET", aws: { signQuery: true, expiresIn: 60 } });
...
export async function deleteObject(key: string): Promise<void> {
  if (!client) throw new Error("R2 not configured");
  const signed = await client.sign(objectUrl(key), { method: "DELETE", aws: { signQuery: true, expiresIn: 60 } });
